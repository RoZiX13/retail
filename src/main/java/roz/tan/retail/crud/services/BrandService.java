...
public class BrandService {
    ...
    public Brand save(BrandInput input) {
        ...
        if (input.getName() == null || input.getName().isBlank()) {
            throw new IllegalArgumentException("Brand name must not be null or blank");
        }

        // 2. Загрузка страны (если указана)
        Country country = null;
        if (input.getCountryId() != null) {
            country = countryRepository.findById(input.getCountryId())
                    .orElseThrow(() -> new EntityNotFoundException("Country not found with id: " + input.getCountryId()));
        }
    ...
    @Transactional(readOnly = false)
    public Brand update(Integer id, BrandInput input) {
        ...
        if (input.getName() != null && !input.getName().isBlank()) {
            if (!input.getName().equals(brand.getName())) {
        ...
        } else if (input.getCountryId() == null && brand.getCountry() != null) {
