...
public class ProductService {
    ...
    public Product save(ProductInput input) {
        ...
        if (input.getSku() == null || input.getSku().isBlank()) {
        ...
        if (input.getPurchasePrice() == null || input.getPurchasePrice() <= 0) {
            throw new IllegalArgumentException("Purchase price must be positive");
        }
        if (input.getSalePrice() == null || input.getSalePrice() <= 0) {
            throw new IllegalArgumentException("Sale price must be positive");
        }
        if (input.getSalePrice() < input.getPurchasePrice()) {
        ...
        product.setActive(input.getIsActive() == null ? true : input.getIsActive());

        // 3. Загрузка и установка Category (опционально)
        if (input.getCategoryId() != null) {
            Category category = categoryRepository.findById(input.getCategoryId())
                    .orElseThrow(() -> new EntityNotFoundException("Category not found with id: " + input.getCategoryId()));
        ...
        if (input.getBrandId() != null) {
            Brand brand = brandRepository.findById(input.getBrandId())
                    .orElseThrow(() -> new EntityNotFoundException("Brand not found with id: " + input.getBrandId()));
            product.setBrand(brand);
        }

        // 5. Загрузка и установка Unit (обязательно)
        Unit unit = unitRepository.findById(input.getUnitId())
                .orElseThrow(() -> new EntityNotFoundException("Unit not found with id: " + input.getUnitId()));
        ...
        product.getSalePrice().add(salePrice);
    ...
    public Product update(Integer id, ProductInput input) {
        ...
        if (input.getUnitId() != null) {
            ...
            unitRepository.findById(input.getUnitId())
                    .orElseThrow(() -> new EntityNotFoundException("Unit not found with id: " + input.getUnitId()));
        }
        if (input.getPurchasePrice() != null && input.getPurchasePrice() <= 0) {
            throw new IllegalArgumentException("Purchase price must be positive");
        ...
        if (input.getPurchasePrice() != null && input.getSalePrice() != null && input.getSalePrice() < input.getPurchasePrice()) {
            throw new IllegalArgumentException("Sale price cannot be less than purchase price");
        }
